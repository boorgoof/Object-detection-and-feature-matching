...
class Dataset{
     ...
     */
    std::map<std::string, std::vector<Label>> test_items; //MODIFY, BUILD A MAP OF string(filepath test image) - label
    /**
     ...
     */
    const std::map<std::string, std::vector<Label>>& get_test_items() const {return this->test_items;}
